#!/usr/bin/env python3
"""
Hardware Component Test Script - WORKING VERSION
Tests all hardware components sequentially for 5 seconds each.

FIXED ISSUES:
- Correct Python path setup (points to Drivers directory)
- Proper import statements
- Better error handling and diagnostics

Usage:
    cd /home/iot/Downloads/Raspi/Tests
    python3 test_hardware_working.py

Or from anywhere:
    python3 /path/to/Raspi/Tests/test_hardware_working.py
...
# CRITICAL FIX: Add Drivers directory to Python path
# The hardware_manager uses relative imports like "from hardware.gpio..."
# So we need to add the Drivers directory, not Raspi directory
script_dir = Path(__file__).resolve().parent
raspi_dir = script_dir.parent
drivers_dir = raspi_dir / "Drivers"

# Add Drivers to path FIRST
...
try:
    ...
    print("✅ Successfully imported hardware_manager")
except ImportError as e:
    print(f"❌ Error importing hardware_manager: {e}")
    print("\n🔍 Troubleshooting:")
    print(f"   - Check that {drivers_dir} exists")
    print(f"   - Check that {drivers_dir}/hardware/managers/hardware_manager.py exists")
    print(f"   - Make sure all __init__.py files are present")
    ...
    print("   └── Tests/")
    print("       └── test_hardware_working.py (this file)")
    sys.exit(1)
...
class HardwareTest:
    ...
    def initialize(self):
        ...
        try:
            ...
            print("\n🔍 Common issues:")
            print("   - GPIO permissions: Run 'sudo usermod -a -G gpio $USER' and reboot")
            print("   - Missing dependencies: Run 'pip3 install -r requirements.txt'")
            print("   - Hardware not connected: Check CONNECTIONS_GUIDE.md")
            print("   - Not on Raspberry Pi: This must run on actual Pi hardware")
            import traceback
            traceback.print_exc()
            return False
    ...
    def test_servos(self):
        """Test servo movements - PAN ONLY (tilt optional)"""
    ...
    def test_ultrasonic(self):
        ...
        try:
            ...
            if valid_readings:
                ...
                print("   ⚠️  No valid readings received")
    ...
    def test_camera(self):
        ...
        try:
            ...
            if frame_count == 0:
                print("   ⚠️  No frames captured - check camera connection")
    ...
    def run_all_tests(self):
        ...
        print("\n" + "="*60)
        print("  🔧 HARDWARE COMPONENT TEST SUITE - WORKING VERSION")
        ...
        try:
            ...
            print("   ✅ Servos (pan left, right, center)")
...
def main():
    """Main entry point"""
    print("\n" + "="*60)
    print("  HARDWARE TEST - WORKING VERSION")
    print("="*60)
    ...
    try:
        with open('/proc/cpuinfo', 'r') as f:
            ...
            if 'Raspberry Pi' not in cpuinfo:
                ...
                print("   This script requires actual Raspberry Pi hardware")
        ...
        print("⚠️  Warning: Could not verify Raspberry Pi (/proc/cpuinfo not found)")
        print("   This script requires actual Raspberry Pi hardware")
        response = input("Continue anyway? (y/n): ")
        if response.lower() != 'y':
            sys.exit(0)
