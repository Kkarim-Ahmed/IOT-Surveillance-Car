...
_CONFIG_PATH = os.path.join(
    os.path.dirname(__file__), "..", "mqtt-config", "client_config.json"
)
...
class SurveillanceCarGUI:
    ...
    def _build_ui(self):
        ...
        title = tk.Label(
            self.root,
            text="🚗  Surveillance Car Controller",
            font=("Helvetica", 16, "bold"),
            bg="#1e1e2e", fg="#cdd6f4",
        )
        title.pack(fill="x", ipady=8)
        ...
        tk.Label(status_frame, text="Broker:", bg="#313244", fg="#a6adc8").pack(side="left", **PAD)
        self._broker_var = tk.StringVar(value="Loading…")
        tk.Label(status_frame, textvariable=self._broker_var, bg="#313244", fg="#89b4fa").pack(side="left")

        tk.Label(status_frame, text="  Status:", bg="#313244", fg="#a6adc8").pack(side="left")
        ...
        tk.Radiobutton(
            status_frame, text="Local", variable=self._mode_var, value="local",
            bg="#313244", fg="#a6e3a1", selectcolor="#313244",
            command=self._switch_broker,
        ).pack(side="right", **PAD)
        tk.Radiobutton(
            status_frame, text="Cloud", variable=self._mode_var, value="cloud",
            bg="#313244", fg="#fab387", selectcolor="#313244",
            command=self._switch_broker,
        ).pack(side="right", **PAD)
        tk.Label(status_frame, text="Mode:", bg="#313244", fg="#a6adc8").pack(side="right")
        ...
        dpad_frame.pack(padx=10, pady=8, fill="x")

        btn_cfg = {"width": 8, "height": 2, "bg": "#45475a", "fg": "#cdd6f4",
        ...
        # Centre the grid columns
        for col in range(3):
            dpad_frame.columnconfigure(col, weight=1)

        # ── Speed slider ──────────────────────────────────────────────────────
        speed_frame = tk.Frame(self.root, bg="#1e1e2e")
        speed_frame.pack(fill="x", padx=10)

        tk.Label(speed_frame, text="Speed:", bg="#1e1e2e", fg="#a6adc8").pack(side="left")
        self._speed_var = tk.IntVar(value=self._current_speed)
        speed_slider = tk.Scale(
            speed_frame, from_=0, to=100, orient="horizontal",
            variable=self._speed_var, bg="#1e1e2e", fg="#cdd6f4",
            troughcolor="#313244", highlightthickness=0, length=200,
            command=self._on_speed_change,
        )
        speed_slider.pack(side="left", padx=6)
        self._speed_label = tk.Label(speed_frame, text="80%", bg="#1e1e2e", fg="#89b4fa")
        self._speed_label.pack(side="left")

        # ── Servo slider ──────────────────────────────────────────────────────
        servo_frame = tk.Frame(self.root, bg="#1e1e2e")
        servo_frame.pack(fill="x", padx=10, pady=4)

        tk.Label(servo_frame, text="Servo:", bg="#1e1e2e", fg="#a6adc8").pack(side="left")
        self._servo_var = tk.IntVar(value=self._current_servo)
        servo_slider = tk.Scale(
            servo_frame, from_=0, to=180, orient="horizontal",
            variable=self._servo_var, bg="#1e1e2e", fg="#cdd6f4",
            troughcolor="#313244", highlightthickness=0, length=200,
            command=self._on_servo_change,
        )
        servo_slider.pack(side="left", padx=6)
        self._servo_label = tk.Label(servo_frame, text="90°", bg="#1e1e2e", fg="#89b4fa")
        ...
        periph_frame.pack(padx=10, pady=4, fill="x")

        pbtn = {"width": 9, "bg": "#45475a", "fg": "#cdd6f4",
                "activebackground": "#a6e3a1", "relief": "raised", "bd": 2}

        tk.Button(periph_frame, text="💡 LED On",  **pbtn,
                  command=lambda: self._send_led("on")).pack(side="left", padx=4, pady=6)
        tk.Button(periph_frame, text="🌑 LED Off", **pbtn,
                  command=lambda: self._send_led("off")).pack(side="left", padx=4, pady=6)
        tk.Button(periph_frame, text="✨ Blink",   **pbtn,
                  command=lambda: self._send_led("blink")).pack(side="left", padx=4, pady=6)
        tk.Button(periph_frame, text="🔔 Beep",    **pbtn,
                  command=self._send_beep).pack(side="left", padx=4, pady=6)
        tk.Button(periph_frame, text="🎯 Center",  **pbtn,
                  command=self._center_servo).pack(side="left", padx=4, pady=6)
        ...
        info_frame.pack(padx=10, pady=4, fill="x")

        self._pi_status_var = tk.StringVar(value="—")
        tk.Label(info_frame, textvariable=self._pi_status_var,
                 bg="#1e1e2e", fg="#a6e3a1", font=("Courier", 9)).pack(
        ...
        log_frame.pack(padx=10, pady=(4, 8), fill="both", expand=True)
    ...
    def _bind_keys(self):
        """Arrow keys / WASD for movement, Q/E for servo, Space to stop."""
        bindings = {
            "<Up>"       : lambda e: self._send_motor("forward"),
            "<Down>"     : lambda e: self._send_motor("backward"),
            "<Left>"     : lambda e: self._send_motor("left"),
            "<Right>"    : lambda e: self._send_motor("right"),
            "<space>"    : lambda e: self._send_motor("stop"),
            "w"          : lambda e: self._send_motor("forward"),
            "s"          : lambda e: self._send_motor("backward"),
            "a"          : lambda e: self._send_motor("left"),
            "d"          : lambda e: self._send_motor("right"),
        }
        for key, handler in bindings.items():
            self.root.bind(key, handler)

        # Key-release → stop (only if no other movement key is held)
        for key in ("<Up>", "<Down>", "<Left>", "<Right>", "w", "s", "a", "d"):
            self.root.bind(f"<KeyRelease-{key.strip('<>')}>" if "<" not in key else
                           key.replace("<", "<KeyRelease-"),
                           lambda e: self._send_motor("stop"))
    ...
    def _connect(self):
        """Build MQTT client and connect using ConnectionManager."""
        ...
        # Subscribe to status topic after connect
    ...
    def _switch_broker(self):
        """Disconnect from current broker and reconnect to the selected one."""
        if self._client:
    ...
    def _on_message(self, client, userdata, msg):
        """Handle incoming dev/status messages from the Pi."""
        try:
    ...
    def _publish(self, topic: str, payload: dict, qos: int = 1):
        """Publish a JSON payload; log a warning if not connected."""
    ...
    def _send_motor(self, direction: str):
        """Publish a motor command to dev/motor."""
        self._publish(TOPIC_MOTOR, {
            "direction": direction,
            "speed"    : self._current_speed,
        })

    def _send_led(self, state: str):
        """Publish an LED command to dev/led."""
        self._publish(TOPIC_LED, {"state": state})

    def _send_beep(self):
        """Publish a BEEP command to dev/commands."""
        self._publish(TOPIC_COMMANDS, {"command": "BEEP"})

    def _center_servo(self):
        """Send servo to 90° center position."""
    ...
    def _adjust_servo(self, delta: int):
        """Nudge servo by delta degrees (keyboard shortcut)."""
    ...
    def _display_pi_status(self, payload: dict):
        """Update the Pi status panel with the latest status message."""
        parts = []
    ...
    def _log_msg(self, message: str, level: str = "info"):
        """Append a timestamped message to the log box (thread-safe)."""
        def _append():
