...
class MotorDashboard:
    ...
    BASE_SPEED = 200
    DEAD_PX    = 30
    REF_H      = 200    # bbox height at 1 m — updated by calibrate()
    ...
    def update(self, bbox: tuple, fw: int = 640, fh: int = 480):
        ...
        bh           = y2 - y1
        self.distance_cm = (self.REF_H * 100.0 / bh) if bh > 0 else 9999.0
        d = self.distance_cm
...
class FaceTrackingGUI:
    ...
    def _update_motor_display(self):
        ...
        # Speed bar values
        rpm_l = int((m.left_speed / 255.0) * 150)
        rpm_r = int((m.right_speed / 255.0) * 150)
