...
class Track:
    def __init__(self, bbox, name="Unknown", conf=0.0):
        ...
        self._signature: Optional[np.ndarray] = None   # upper-body hue histogram
    ...
    def update_signature(self, frame: np.ndarray) -> None:
        """Store a hue histogram of the upper-body region for crowd re-ID."""
        x1, y1, x2, y2 = self.bbox()
        mid_y = (y1 + y2) // 2
        crop = frame[max(0, y1):max(0, mid_y), max(0, x1):max(0, x2)]
        if crop.size == 0:
            return
        hsv  = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV)
        hist = cv2.calcHist([hsv], [0], None, [32], [0, 180])
        cv2.normalize(hist, hist)
        self._signature = hist.flatten()

    def signature_similarity(self, other: "Track") -> float:
        """Returns [0, 1] colour similarity. 0.5 if either has no signature."""
        if self._signature is None or other._signature is None:
            return 0.5
        return float(cv2.compareHist(
            self._signature, other._signature, cv2.HISTCMP_CORREL))
...
class Tracker:
    ...
    def update(self, detections: list, names=None,
        ...
        if self.tracks and detections:
            ...
            for flat in np.argsort(-iou_mat, axis=None):
                ...
                if iou_mat[ti, di] >= config.IOU_THRESHOLD:
                    ...
                    # Update appearance signature when a named track is refreshed
                    if frame is not None and n != "Unknown":
                        self.tracks[ti].update_signature(frame)
    ...
    def predict_only(self) -> List[Track]:
        """Advance Kalman state without updating detections or aging tracks."""
    ...
    # Set this to a name string to lock the primary to a specific person.
    # Pipeline sets it on first successful recognition; clear to None to re-acquire.
    ...
    def primary(self) -> Optional[Track]:
        if not self.tracks:
            return None
        # 1. Target locked — prefer that name, score ties by conf + appearance
        if self.target_name:
            locked = [t for t in self.tracks if t.name == self.target_name]
            if locked:
                ref = max(locked, key=lambda t: t.confidence)
                if len(locked) > 1:
                    # Multiple tracks with same name (rare) — pick best conf×appearance
                    return max(locked, key=lambda t: t.confidence * 0.7
                               + ref.signature_similarity(t) * 0.3)
                return ref
        # 2. No lock — prefer named track with highest confidence
        named = [t for t in self.tracks if t.name != "Unknown" and t.confidence > 0]
        if named:
            best = max(named, key=lambda t: t.confidence)
            if self.target_name is None:
                self.target_name = best.name
                log.info("Target locked: %s (conf=%.2f)", best.name, best.confidence)
            return best
        # 3. Nobody recognized yet — follow largest (closest)
        return max(self.tracks, key=lambda t: (
