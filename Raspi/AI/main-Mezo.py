...
# ════════════════════════════════════════════════════════════════════════════
# Vertical speed-bar widget
# ════════════════════════════════════════════════════════════════════════════

class SpeedBar(tk.Canvas):
    """Dark-themed vertical bar that visualises a motor speed value (0-255)."""
    ...
    def __init__(self, parent, label: str = "", **kw):
        super().__init__(parent,
                         width=self.BAR_W, height=self.BAR_H + 16,
    ...
    def set_value(self, v: int, color: str = GREEN) -> None:
        self._value = max(0, min(self.MAX_VAL, int(v)))
        self._color = color
        self._draw()
    ...
    def _draw(self) -> None:
        ...
        # Track
        self.create_rectangle(1, 1, W - 1, H, fill=SURFACE, outline=BORDER)
        # Fill (bottom-up)
        fill_h = int(self._value / self.MAX_VAL * (H - 2))
        if fill_h > 0:
            self.create_rectangle(2, H - fill_h, W - 2, H - 1,
...
class CalibrationDialog(tk.Toplevel):
    """
    Live-updating dialog that lets the user calibrate the distance reference.
    Shows the current bounding-box height so the user can confirm their
    position before committing.
    """

    POLL_MS = 100   # refresh interval

    def __init__(self, parent, pipeline, get_tracks_fn):
        super().__init__(parent)
        self.title("Calibrate Distance")
        self.configure(bg=BG)
        self.resizable(False, False)
        self.grab_set()

        self._pipeline      = pipeline
        self._get_tracks    = get_tracks_fn   # callable → list[dict]
        self._running       = True
        self._current_bbox_h = 0

        self._build_ui()
        self._poll()

    def _build_ui(self):
        tk.Label(self, text="Distance Calibration",
                 bg=BG, fg=TEXT, font=("Segoe UI", 13, "bold")).pack(pady=(16, 4))

        tk.Label(self,
                 text="Stand exactly 1 metre from the camera,\nthen press 'Set as 1 m Reference'.",
                 bg=BG, fg=TEXT_DIM, font=("Segoe UI", 10), justify="center").pack(padx=24, pady=4)

        # Live readout panel
        info = tk.Frame(self, bg=PANEL, padx=16, pady=12)
        info.pack(padx=24, pady=8, fill="x")

        tk.Label(info, text="Live bbox height:",
                 bg=PANEL, fg=TEXT_DIM, font=("Segoe UI", 10)).pack(side="left")

        self._h_var = tk.StringVar(value="waiting for face…")
        tk.Label(info, textvariable=self._h_var,
                 bg=PANEL, fg=BLUE, font=("Segoe UI", 12, "bold")).pack(side="right")

        # Buttons
        btn_row = tk.Frame(self, bg=BG)
        btn_row.pack(pady=12)

        self._set_btn = tk.Button(btn_row, text="✓  Set as 1 m Reference",
                                   command=self._commit, state="disabled")
        _style_btn(self._set_btn, bg=BTN_GREEN)
        self._set_btn.pack(side="left", padx=8)
    ...
    def _poll(self):
        ...
        if tracks:
            ...
            self._current_bbox_h = y2 - y1
            self._h_var.set(f"{self._current_bbox_h} px")
            self._set_btn.configure(state="normal")
        else:
            self._h_var.set("no face detected")
            self._set_btn.configure(state="disabled")
        self.after(self.POLL_MS, self._poll)

    def _commit(self):
        motor = self._pipeline.motor
        if motor and self._current_bbox_h > 0:
            motor.calibrate(self._current_bbox_h)
            messagebox.showinfo(
                "Calibrated",
                f"Reference set: {self._current_bbox_h} px = 1 m\n"
                "Distance estimation is now active.",
                parent=self,
            )
        self.destroy()
...
class App(tk.Tk):
    ...
    def __init__(self, pipeline, result_queue: queue.Queue):
        ...
        # Motor telemetry vars
        self._motor_dist_var  = tk.StringVar(value="–")
        self._motor_errx_var  = tk.StringVar(value="–")
        self._motor_lspd_var  = tk.StringVar(value="0")
        self._motor_rspd_var  = tk.StringVar(value="0")
        self._motor_state_var = tk.StringVar(value="IDLE")
        self._motor_estop     = False   # emergency-stop toggle state

        # SpeedBar widget references (set during _build_sidebar)
        self._lspd_bar: "SpeedBar | None" = None
        self._rspd_bar: "SpeedBar | None" = None
    ...
    def _build_sidebar(self, parent):
        ...
        # ── MOTOR TELEMETRY ───────────────────────────────────────────────────
        motor_card = self._card(sb, "MOTOR")
        motor_card.pack(fill="x", pady=(0, 8))

        # Zone indicator (coloured state pill)
        zone_row = tk.Frame(motor_card, bg=PANEL)
        zone_row.pack(fill="x", pady=(0, 4))
        tk.Label(zone_row, text="Zone", bg=PANEL, fg=TEXT_DIM,
                 font=("Segoe UI", 9), width=10, anchor="w").pack(side="left")
        self._motor_state_lbl = tk.Label(zone_row, textvariable=self._motor_state_var,
                                          bg=PANEL, fg=ORANGE,
                                          font=("Segoe UI", 9, "bold"), anchor="e")
        self._motor_state_lbl.pack(side="right")

        # Distance + error X text rows
        for label, var in [("Distance", self._motor_dist_var),
                            ("Error X",  self._motor_errx_var)]:
            row = tk.Frame(motor_card, bg=PANEL)
            row.pack(fill="x", pady=1)
            tk.Label(row, text=label, bg=PANEL, fg=TEXT_DIM,
                     font=("Segoe UI", 9), width=10, anchor="w").pack(side="left")
            tk.Label(row, textvariable=var, bg=PANEL, fg=TEXT_DIM,
                     font=("Segoe UI", 9, "bold"), anchor="e").pack(side="right")

        # Speed bars + numeric values
        spd_outer = tk.Frame(motor_card, bg=PANEL)
        spd_outer.pack(fill="x", pady=(6, 4))

        bar_frame = tk.Frame(spd_outer, bg=PANEL)
        bar_frame.pack(side="left")
        self._lspd_bar = SpeedBar(bar_frame, label="L")
        self._lspd_bar.pack(side="left", padx=(0, 6))
        self._rspd_bar = SpeedBar(bar_frame, label="R")
        self._rspd_bar.pack(side="left")

        num_frame = tk.Frame(spd_outer, bg=PANEL)
        num_frame.pack(side="left", padx=(8, 0))
        for lbl, var in [("L:", self._motor_lspd_var), ("R:", self._motor_rspd_var)]:
            row = tk.Frame(num_frame, bg=PANEL)
            row.pack(fill="x", pady=2)
            tk.Label(row, text=lbl, bg=PANEL, fg=TEXT_DIM,
                     font=("Segoe UI", 9), width=3, anchor="w").pack(side="left")
            tk.Label(row, textvariable=var, bg=PANEL, fg=TEXT,
                     font=("Segoe UI", 9, "bold"), anchor="e").pack(side="right")

        # Action buttons
        motor_btn_row = tk.Frame(motor_card, bg=PANEL)
        motor_btn_row.pack(fill="x", pady=(4, 2))

        calib_btn = tk.Button(motor_btn_row, text="📏  Calibrate Distance",
                               command=self._calibrate_distance)
        _style_btn(calib_btn, bg=BTN_BLUE, font_size=9)
        calib_btn.pack(side="left", expand=True, fill="x", padx=(0, 3))

        self._estop_btn = tk.Button(motor_btn_row, text="⛔  E-STOP",
                                     command=self._emergency_stop)
        _style_btn(self._estop_btn, bg=BTN_RED, font_size=9)
        self._estop_btn.pack(side="left", expand=True, fill="x", padx=(3, 0))
    ...
    def _update_frame(self):
        try:
            ...
            if ms:
                ...
                dist_str = f"{dist:.0f} cm" if dist < 9000 else "–"
                self._motor_dist_var.set(dist_str)
                self._motor_errx_var.set(f"{ms.get('error_x', 0.0):.0f} px")
                self._motor_lspd_var.set(str(l_spd))
                self._motor_rspd_var.set(str(r_spd))
                self._motor_state_var.set(state)

                zone_colors = {
                    "STOP":    RED,    "HOLD":    ORANGE,
                    "FOLLOW":  GREEN,  "CHASE":   BLUE,
                    "FORWARD": GREEN,  "TURN":    ORANGE,  # backward compat
                    "IDLE":    TEXT_DIM,
                }
                bar_colors = {
                    "STOP":    BTN_RED,   "HOLD":    ORANGE,
                    "FOLLOW":  GREEN,     "CHASE":   BLUE,
                    "FORWARD": GREEN,     "TURN":    ORANGE,
                    "IDLE":    TEXT_DIM,
                }
                col     = zone_colors.get(state, TEXT_DIM)
                bar_col = bar_colors.get(state, BLUE)
                self._motor_state_lbl.configure(fg=col)

                if self._lspd_bar:
                    self._lspd_bar.set_value(l_spd, color=bar_col)
                if self._rspd_bar:
                    self._rspd_bar.set_value(r_spd, color=bar_col)
    ...
    def _emergency_stop(self):
        ...
        if motor is None:
            return
        if self._motor_estop:
            # Resume
            motor.resume()
            self._motor_estop = False
            self._estop_btn.configure(text="⛔  E-STOP", bg=BTN_RED)
            self._motor_state_var.set("IDLE")
            self._motor_state_lbl.configure(fg=TEXT_DIM)
            if self._lspd_bar: self._lspd_bar.set_value(0, color=TEXT_DIM)
            if self._rspd_bar: self._rspd_bar.set_value(0, color=TEXT_DIM)
        else:
            # Stop
            motor.emergency_stop()
            self._motor_estop = True
            self._estop_btn.configure(text="▶  RESUME", bg=BTN_GREEN)
            self._motor_lspd_var.set("0")
            self._motor_rspd_var.set("0")
            self._motor_state_var.set("STOP")
            self._motor_state_lbl.configure(fg=RED)
            if self._lspd_bar: self._lspd_bar.set_value(0, color=BTN_RED)
            if self._rspd_bar: self._rspd_bar.set_value(0, color=BTN_RED)
    ...
    def _calibrate_distance(self):
        ...
        if motor is None:
            messagebox.showwarning(
                "Motor Disabled",
                "Motor controller is not enabled.\n"
                "Set MOTOR_ENABLED = True in config.py.",
                parent=self,
            )
            return
        CalibrationDialog(self, self._pipeline, lambda: self._last_tracks)
...
def parse_args():
    ...
    p.add_argument("--mode",   choices=["detect", "track"], default=config.DEFAULT_MODE)
